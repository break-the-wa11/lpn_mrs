...
from normflows_1D.flows.base import Flow, zero_log_det_like_z
from normflows_1D.flows.reshape import Split, Merge
...
class AffineConstFlow(Flow):
    ...
    def __init__(self, shape, scale=True, shift=True):
        ...
        super().__init__()
        if scale:
            self.s = nn.Parameter(torch.zeros(shape)[None])     # new shape: (1, shape)
        else:
        ...
        self.n_dim = self.s.dim()   #n_dim = len(shape) + 1
        self.batch_dims = torch.nonzero(
            torch.tensor(self.s.shape) == 1, as_tuple=False
        )[:, 0].tolist()    # all dimensions with size 1 are identified as batch dimensions
